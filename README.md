...
```bash
# 1. 在能上网的机器上，先把所有 binary + image + chart 拉到本地 third_party/
#    (v0.0.5+ mega-bundle：containerd + kubeadm + kubelet + cri-dockerd + docker
#     + registry + k8s/cilium/prometheus 镜像 + cilium/prometheus helm chart)
ko vendor fetch

# 2. 把 third_party/ 烤成单一 bundle tar.gz
ko pack build --arch all --output ./dist --version bundle-k8s1.32.0-cilium1.16.1-20260702
# 产物：dist/bundle-k8s1.32.0-cilium1.16.1-20260702-multi.oci.tar.gz
...
ko vendor fetch|clean|paths        维护 third_party/（fetch 是唯一会联网的步骤）
